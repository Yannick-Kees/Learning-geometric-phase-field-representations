...
def make_color_plot(n):
    f = np.load(open(r"dataset/dataset_faces100.npy", "rb"),allow_pickle=True)
    ...
    dist = chamfer_distance(Tensor(np.array(lista)),Tensor(np.array(listb)), batch_reduction=None)[0].numpy().reshape(n,n)
    sns.heatmap(dist)
    plt.savefig('scattering' + str(n) +'.png')
